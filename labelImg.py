...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def set_format(self, save_format):
        if save_format == FORMAT_PASCALVOC:
            ...
            self.actions.save_format.setIcon(newIcon("format_pick"))
            self.labelFileFormat = LabelFileFormat.PICK
            LabelFile.suffix = TSV_EXT
    ...
    def showBoundingBoxFromAnnotationFile(self, filePath):
        if self.defaultSaveDir is not None:
            ...
            elif os.path.isfile(tsvPath):
                self.loadPICKTSVByFilename(tsvPath)
        else:
            xmlPath = os.path.splitext(filePath)[0] + XML_EXT
            txtPath = os.path.splitext(filePath)[0] + TXT_EXT
            tsvPath = os.path.splitext(filePath)[0] + TSV_EXT
            if os.path.isfile(xmlPath):
                self.loadPascalXMLByFilename(xmlPath)
            elif os.path.isfile(txtPath):
                self.loadYOLOTXTByFilename(txtPath)
    ...
    def loadPICKTSVByFilename(self, tsvPath):
        if self.filePath is None:
            return
        ...
        print (shapes)
        self.loadLabels(shapes)
        self.canvas.verified = tPICKParseReader.verified
